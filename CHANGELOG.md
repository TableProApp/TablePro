...
- Caret at the end of a query lost its line-number color highlight in the gutter even though the line background stayed shaded. `drawLineNumbers` matched lines via `IndexSet.intersects(integersIn: lineRange)` (half-open), so `cursorOffset == documentLength` never intersected. Empty selections now route through `textLineForOffset`, mirroring how `drawSelectedLines` already handles end-of-document carets.
- Cmd+Left/Right and Shift+Cmd+Left/Right did nothing when the SQL editor cursor sat at, or one position before, the end of a line that has no trailing newline (i.e. the end of the query). `extendSelectionVisualLine` looked up the visual line fragment with `getLine(atOffset:)`, which returns nil for an offset equal to the line length, and unconditionally subtracted the document's detected line-ending length even on the last line, capping the right-hand bound one short. The visual-line lookup now falls back to the last fragment, and the line-ending length is computed from the actual line's trailing characters, matching how `extendSelectionLine` already handles real lines. Fixes #1007.
...
- Terminology aligns with macOS HIG: "Read-Only" / "Read-Write" become "Read Only" / "Read & Write" in `ExternalAccessLevel` and `SafeModeLevel` display names; sentence-level "read-only" usages across MCP, SafeMode, and toolbar strings drop the hyphen.
- Database type chooser sheet for new connections. Clicking "New Connection" now opens a 480×560 modal sheet (Apple Internet Accounts pattern) with a categorised, search-enabled list of all 14+ supported database drivers grouped by `DatabaseCategory` (Relational, Document, Key-Value, Analytical, Wide-Column, Cloud Native, Coordination & Config). Each row shows the driver icon, name, and a one-line tagline. Editing an existing connection skips the chooser; in-form "Change…" reopens it.
