...
- Linked SQL Folders v1.1: inline metadata dialog (`Edit Metadata...` from the sidebar context menu) for editing `@name` / `@keyword` / `@description` without opening the file in the editor. Live "modified on disk" banner appears at the top of an editor tab when a linked file changes externally, with a one-click Reload. Save-time conflict prompt now shows a side-by-side diff sheet (line-level, via `Swift.CollectionDifference`) instead of an alert. Linked favorites and regular favorites are draggable: drop a row onto the SQL editor to insert its content. Non-UTF-8 files are detected, loaded with `String(contentsOf: usedEncoding:)`, and surfaced in the sidebar with a yellow warning icon; saving preserves the file's original encoding (UTF-16, ISO Latin-1, etc.). UTF-8 BOM bytes at the start of a frontmatter file no longer cause metadata to be silently ignored. Folder management moved from Settings into the Favorites sidebar: the bottom `+` button opens a popup with `New Favorite` / `New Folder` / `Add Linked SQL Folder...`, matching the Mail.app sidebar pattern, and right-click on a linked folder offers Disable, Reload, Copy Path, Show in Finder, and Remove from Sidebar.
