...
- The Cloud SQL Auth Proxy and the Copilot language server are checked against the version and checksum TablePro recorded for them before they are run, and replaced when either no longer matches. A helper already on disk was run without either check, so a helper updated in a new TablePro release never reached a Mac that already had the old one, and a helper altered afterwards kept being run.
