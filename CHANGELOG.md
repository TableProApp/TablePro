...
- Manage database users and roles on MySQL and PostgreSQL connections. A **Users & Roles** tab lists the accounts on the server and shows each one's privileges as a tree you can expand from the server down through databases, schemas, tables, and columns, with a checkbox per privilege at every level. Create, drop, and set passwords, and grant or revoke privileges. Every change is shown as the exact SQL it will run before it runs. (#1413)
