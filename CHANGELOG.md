...
- Manage database users and roles on MySQL and PostgreSQL connections. A **Users & Roles** tab lists the accounts on the server and shows each one's server and per-database privileges in a checkbox grid. Create, drop, and set passwords, and grant or revoke privileges. Every change is shown as the exact SQL it will run before it runs. Table and column privileges an account already has are shown read-only so a database-level change cannot hide them. (#1413)
