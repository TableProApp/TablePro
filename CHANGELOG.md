...
- Open Quickly is drawn on the system Liquid Glass material on macOS 26 and later, instead of a hand-placed effect view with a border painted over it, so the panel reads as one surface. Its rows, headings and hints use the system text styles rather than hard-coded point sizes. On macOS 14 and 15 the panel keeps the blurred popover material it always had.
- Open Quickly's scope filters moved inside the panel as labelled chips and stay visible at all times. They were round buttons outside the panel that vanished as soon as anything was listed, which on any connection with history was the first frame, so four of the five scopes had no way to be picked with the mouse.
- Open Quickly has a footer spelling out what `Return`, `Cmd+Return`, `Cmd+1` to `Cmd+5` and `Escape` do. The `Escape` hint reads **Clear** while you have something typed and **Close** when the field is empty, so the first `Escape` no longer looks like a key that did nothing.
...
- Exporting a database other than the one you were browsing opened a dialog that listed a different database's tables. On PostgreSQL, SQL Server, Redshift, CockroachDB, SurrealDB, Snowflake and Trino, right-clicking a database and choosing Export showed the active database's schemas with nothing selected, while naming the file after the database you picked. The dialog now reads and exports from the database you chose, with everything in it selected. On DuckDB and PGlite, where a second connection cannot reach another database, Export is offered only for the active one instead of opening a dialog that cannot answer.
- On Oracle, Dameng and BigQuery, "Close Tabs for Other Databases" closed every query tab you had open, with no warning, because a query tab belongs to no schema and the command read that as belonging to another one. It now closes only tabs that are in a different schema, and leaves a tab that is in none alone. The menu item reads "Close Tabs for Other Schemas" on those connections, and Open Database reads "Open Schema", which both already had wording for but never used.
- A `tablepro://` link that named a database applied that name as a schema on PostgreSQL and every other connection that has schemas, so the link left the session on its old database with a search path pointing at a schema that does not exist. Nothing reported an error, the object list emptied, and the bad value was remembered and reapplied on every later reconnect. A link now applies the database first and the schema second, whichever of them it names, and a segment naming a dimension the connection does not have is ignored instead of being applied to the other one.
- On PostgreSQL there was no way to change the active schema without going through the menu bar. The toolbar showed the schema name and its tooltip offered to switch it, but clicking it opened the database list. The toolbar now shows the database and the schema side by side, each opening its own list, and the schema picker is back at the foot of the object list. Redshift, CockroachDB, PGlite, SQL Server, DuckDB, Snowflake and Trino were affected the same way. (#2196)
- The foreign-key arrow in a data-grid cell arrived a moment after the rows, and widened its column as it appeared, so text you were already reading shifted sideways. The arrow is now known before the first row is drawn, because a connection reads its schema's foreign keys once when it loads the object list instead of asking again for each table you open, and it reads them again after you refresh the object list or switch database. Where that is not possible, such as the first table on a driver that cannot answer in one query, the arrow still appears a moment later but no longer moves the column. A column you resized yourself also keeps the width you gave it when you reopen the table, whether or not the arrow is showing. The same applies to the dropdown chevron on enum columns.
- Open Quickly reuses the object list it already built instead of rebuilding it every time the panel opens. Closing and reopening it used to re-query the database and schema lists and re-read 200 rows of query history for a result that had not changed, which on a connection over an SSH tunnel is the whole time the panel sits empty. The list is rebuilt when anything it is built from changes: the database or schema you are browsing, the sidebar's database filter, the set of databases and schemas that exist, your saved queries, or your query history. Disconnecting always rebuilds it, so a list left half-loaded by a disconnect is never shown to the reconnected session.
...
- An open cell editor could save its value onto a different row. Leaving the array editor, JSON editor, hex editor, date picker or type picker open while the rows were replaced, by sorting, refreshing, or applying a filter, made the next save write to whichever row had moved into that position. A JSON editor popped out into its own window had the same problem and outlasted every other way of closing it. Those editors now close when the rows change, as does the column value filter, whose list of values no longer matched the grid.
- Open Quickly reuses the object list it already built instead of rebuilding it every time the panel opens. Closing and reopening it used to re-query the database and schema lists and re-read 200 rows of query history for a result that had not changed, which on a connection over an SSH tunnel is the whole time the panel sits empty. The list is rebuilt when the database, schema, database filter, saved queries or query history actually change.
- Materialized views, foreign tables, system tables, partitioned tables and external tables never appeared in Open Quickly's Recent section and never ranked higher for being used often, however many times you opened them. Only plain tables and views did. Recent also tells apart two objects with the same name in different schemas now.
...
- Turning Caps Lock on stopped every Open Quickly keyboard command from working. `Cmd+Return`, `Cmd+1` to `Cmd+5` and `Ctrl+J`, `Ctrl+K`, `Ctrl+N`, `Ctrl+P` all did nothing until Caps Lock was turned off again, with no error and no beep. The numeric keypad's Enter never worked with `Cmd` at all, whatever Caps Lock was doing.
