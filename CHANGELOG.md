...
- Open Quickly's scope filters moved inside the panel as a standard segmented control and stay visible at all times. They were round buttons outside the panel that vanished as soon as anything was listed, which on any connection with history was the first frame, so four of the five scopes had no way to be picked with the mouse. The segmented control is the control macOS uses for a search scope bar, so the scopes now match every other picker in the app instead of being drawn by hand.
