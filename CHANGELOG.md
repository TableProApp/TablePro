...
- AI Chat: streaming responses no longer hang the process at 100% CPU. Each 150 ms token flush was running `existing + text` on the assistant turn's text block (O(n) per flush, O(n²) over the response), rebuilding `plainText` from blocks on every observer tick, and feeding the result to `Markdown(text)` keyed by `\.offset`, which made MarkdownUI reparse the full message every flush. The streaming bubble now writes tokens to an `@ObservationIgnored` buffer published through a tick counter, renders as plain `Text` while streaming, and commits into a stable-id text block (with cached `plainText`) only at end-of-stream or tool boundary. Schema prefetch and turn resolution move to `Task.detached` so the main thread stays free between flushes (#1205)
