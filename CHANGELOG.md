...
- Settings window rebuilt around macOS Sequoia System Settings pattern. Replaces the pre-Ventura `TabView + .tabItem` shell (horizontal toolbar tabs at top, fixed 720×500) with `NavigationSplitView`: sidebar `List(selection:)` of nine flat `Label(title, systemImage:)` rows (General / Appearance / Editor / Keyboard / AI / Terminal / Integrations / Plugins / Account), `NativeSearchField` at the top of the sidebar that filters items by display name + per-tab keyword metadata, detail switch on enum selection. Window is resizable via `.windowResizability(.contentMinSize)` with a default 820×600, all three traffic light buttons visible. Selected pane persists across launches via the existing `@AppStorage("selectedSettingsTab")` key, preserving the AppDelegate deep-link path for plugin-rejection alerts. Appearance pane rebuilt as a `NavigationStack` drill-down: theme list as Form Section rows with `NavigationLink(value: theme.id)` pushing `ThemeEditorView` as destination, action menu (`+`, Import…) lifted to the toolbar. Plugins pane wraps in `NavigationStack` with the Installed/Browse Picker hoisted to `.toolbar(.principal)`. Across panes, manual `HStack(label, control)` rows replaced with `LabeledContent` (Keyboard shortcut rows, AI provider model row, Linked Folders rows, MCP numeric fields no longer fixed-width).
