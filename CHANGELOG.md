...
- SELECT queries without their own LIMIT now run with the row cap applied as a real LIMIT on the statement sent to the server, so forgetting a LIMIT no longer pulls millions of rows. The query text in the editor never changes, a query that already limits itself is sent as written, and Execute Query Without Limit (Option+Cmd+Enter, also in the Query menu and the Execute button menu) skips the cap for one run. (#1794)
...
- Execute All Statements now applies the query result row cap to each SELECT in the batch instead of returning every row, and capped batch results show the Fetch All link. (#1794)
- Queries starting with a comment are now classified correctly: the row cap applies to them, and Safe Mode recognizes comment-prefixed writes and destructive statements instead of letting them through unprompted.
