...
**Additive changes are binary-compatible in one direction only, and still need a version bump.** Adding a requirement with a default implementation, reordering requirements, or adding a field to a non-`@frozen` transfer struct all keep an *already-built* plugin loading under a *newer* app, which is what Library Evolution covers. The reverse breaks: a plugin **rebuilt** against the new API emits undefined references to the new method descriptor and to the default-implementation symbol itself, so an older app accepts it (`validateBundleVersions` only rejects a version above its own) and then fails `Bundle.loadAndReturnError`, taking that driver away. Measured on a rebuilt `CassandraDriver` and a rebuilt `ClickHouseDriver`; `nm -u <plugin binary> | swift demangle` lists the symbols. So bump `currentPluginKitVersion` for an additive change too, leave `minimumCompatiblePluginKitVersion` where it is so existing plugins keep loading, and the older app then refuses the new plugin cleanly and says to update. Versions 21, 22 and 23 were each such a bump. (#2597)
...
**A synced CKRecord record type or field must be deployed to Production before anything writes it**: both apps pin `com.apple.developer.icloud-container-environment` to `Production`, and CloudKit only auto-creates record types and fields in the Development environment. So no build, not even a local Debug one, can create either on the server. Saving a record that carries a field the Production schema does not declare makes CloudKit reject **that whole record**; saving a record whose *type* is undeclared fails with "Cannot create new type X in production schema". With `isAtomic = false` the rest of the batch still saves, so the symptom is one record type silently never syncing while everything else looks healthy, and because a rejected record never clears its dirty flag it is retried on every sync forever.

Every wire key is declared by a `SyncSchemaField` enum in `Packages/TableProCore/Sources/TableProSyncTransport/` (`ConnectionSyncField` and the rest in `SyncSchemaFields.swift`), and writes go through `record.fields(SomeSyncField.self)`, whose gated subscript refuses any field that is not `.verified`. Never reach a CKRecord field by raw string: that bypasses the gate, which is exactly how `FavoriteTable`, `SQLFavorite`, `SQLFavoriteFolder` and `SSHProfile.jumpHostsJson` reached users undeployed. `SyncMapperFieldAccessTests` scans both mappers for `record["` and fails on a match. `SyncRecordType.verifiedInProduction` gates whole record types the same way, and `CloudKitSyncEngine.push` withholds any record whose type is not verified rather than letting the server reject it. Verification is a membership set, not a `default:` arm, so a case added without being listed is `.unverified` and therefore inert rather than destructive.

To ship one: add the record type or field in CloudKit Console (or `xcrun cktool import-schema --environment development`, the only environment `cktool` can write), deploy Development to Production in the Console, run `scripts/export-cloudkit-schema.sh`, commit the refreshed `CloudKit/production-schema.ckdb`, then add it to the matching `verifiedInProduction`. `ProductionSchemaParityTests` checks every record type and every field against the snapshot in both directions, so it fails both when the app would write something undeployed and when the gate is needlessly withholding something that is deployed. This shipped as `isFavorite` (#1452, unconditional on every connection) killing every Mac connection push for two months while the UI reported success (#643), and again as three favorite record types that never synced at all.
...
**The data grid header owns all of its own chrome, so nothing may ask AppKit to paint any of it**: `NSTableHeaderCell` and `NSTableHeaderView` both paint a fixed 28pt band that they centre vertically in whatever frame they are given, a 16pt column divider on `midY` and a 1pt rule at `midY + 13`. The data grid grows its header to 42pt for a column comment, so that band lands mid-cell: the rule crosses the comment's descenders and sits 8pt above the real bottom edge. `SortableHeaderChrome` is therefore the single owner of header geometry and colours, `SortableHeaderCell.draw(withFrame:in:)` never calls `super`, and `SortableHeaderView.draw(_:)` fills the background and rules the bottom edge itself. The trap is that the header view paints a second copy of that same band for `NSTableView.highlightedTableColumn`, driven by *state* rather than by a drawing call, so no cell override can reach it: setting it gives the sorted column a stray divider and a rule no other column has. TablePro already draws the sorted-column affordance itself (bold title, chevron, priority number, with `drawSortIndicator` overridden to nothing), so `highlightedTableColumn` is a redundant second channel and must stay unset. All sorted-column presentation goes through `SortableHeaderView.applySortState(_:schema:)`, which mirrors the order onto `tableView.sortDescriptors` and updates the cells. Two things about that array. It paints nothing, measured, so it is safe. But it is **not** the accessibility channel either, which this paragraph used to claim: setting it leaves every header cell's `accessibilitySortDirection` at `.unknown`, and `NSAccessibilitySortDirectionAttribute` is only fed by `headerCell.setAccessibilitySortDirection(_:)`, which `applySortIndicators` now calls per column and clears on the rest. AppKit maintains `sortDescriptors` itself on every unmodified header click, because `DataGridColumnPool` gives each data column a `sortDescriptorPrototype`, and a plain click on a second column *prepends* its descriptor while keeping the earlier ones, so the mirror writes the whole list rather than comparing only `.first`. That prototype is also the native knob for the first-click direction (`NSSortDescriptor(key:ascending:)`), and it is what AppKit's own drag suppression keys off: during an aborted header drag a `mouseDragged` override is never called and column order and widths do not change, so comparing `sortDescriptors` across `super.mouseDown` is the only reliable click-versus-drag signal for an unmodified click. `SortableHeaderRenderingTests` rasterises the header and guards this. This shipped as a rule through the comment line and a stray divider on the sorted column (#2017). The pinned row-number heading follows the same rule: `SortableHeaderView.draw(_:)` paints it by running the heading's own fill and `super.draw` again, translated to the visible leading edge and clipped there. A separate view laid over the header can never match, because on macOS 27 `NSTableHeaderView.draw` lays its own translucent grey over the fill and nothing but the header view goes through it; the cap that shipped with the pinned gutter (#2672) rendered at 30 grey beside headings at 50. And scroll the grid through `NSView.scroll(_:)` or AppKit's own scrolling, never `clipView.scroll(to:)` plus `reflectScrolledClipView`: measured on macOS 27, that pair moves the rows and leaves the header clip where it was, so every heading sits off its column.

**The data grid body owns its column separators too, because AppKit's do not scale**: `NSTableView` draws a vertical grid line by keeping one separator view per column as its own subview, and every layout pass re-sorts that whole subview list with an `-[NSArray containsObject:]` comparator inside the sort. That is O(columns) views and O(columns squared) work per pass. Measured on a 500-column result: one layout pass costs 518ms with `.solidVerticalGridLineMask` and 0.03ms with the mask cleared, and 535 of the table view's 536 subviews are separators. Adding or removing any subview of the table view forces such a pass, and the inline cell editor does exactly that on the way in and on the way out, so opening an editor cost about a second (#2381). Only the vertical mask behaves this way: `.solidHorizontalGridLineMask` adds no subviews at all, because AppKit draws a horizontal separator inside `NSTableRowView.drawSeparator(in:)`. So `gridStyleMask` stays empty and `DataGridBodyChrome` is the single owner of separator geometry, thickness and colour, exactly as `SortableHeaderChrome` is for the header. Where it is called from is not a free choice: a row view covers whatever the table view drew beneath it, measured, so `NSTableView.drawGrid(inClipRect:)` and `drawBackground(inClipRect:)` are both invisible behind the rows and visible only past the last row. A row therefore paints the separators crossing it, in a second pass after its cells so a modified or find-match tint cannot paint over one, and `KeyHandlingTableView.drawBackground(inClipRect:)` paints only the area below the last row. The separator stands at each presented column's *leading* edge, which is where AppKit put it and which keeps the row-number column's boundary; take the boundary from `presentsColumn` and `rect(ofColumn:)`, never from a fixed step, per the column rule above. The pinned row gutter is part of the same geometry: it is as wide as `rect(ofColumn:)` of the row-number column, intercell spacing included, and draws its edge through `DataGridBodyChrome` over the unscrolled leading strip, so at scroll offset zero its line is the first data column's line on the same pixel. Sized to `column.width` it stood one point short, and two translucent lines side by side read as one rule twice as thick. It is opaque over its whole height, the table's background first and the row's stripe, tint and selection blended over it, because the dark alternate stripe is white at under 5% alpha and a strip filled with it alone lets the scrolled columns show through. Past the last row it paints the stripes the grid continues there and takes no clicks. Those stripes are the grid's own: `KeyHandlingTableView.drawBackground(inClipRect:)` paints them through `DataGridBodyChrome.drawTableBackground(in:of:)` and never calls `super`, because `NSTableView` blends the stripe in twice past the last row (measured in dark mode: 52 against a row's 40), which read as empty rows brighter than the rows above them and which nothing painting the stripe once can match.
...
**No fixed position in `tableColumns` names a data column**: the attached order is `[__rowNumber__, data columns, surplus pool slots]`, with user-hidden columns among the data columns, so `presentsColumn` is the question to ask, with `firstPresentedColumnIndex` and its neighbours beside it on `DataGridColumnPool`. The rule outlived the column window that made it urgent: that window put a spacer column on each side of the run, `DataGridView.firstDataTableColumnIndex` was a hardcoded `1` that the leading spacer took over, and `isDataTableColumn` accepted the trailing spacer at the other end. The cell cursor was seeded onto a spacer whenever the selection moved without a click, so Down then Return did nothing on any table while the Edit menu item still validated as enabled, Tab out of a row's last cell and Shift+Tab out of its first were swallowed, and `scrollColumnToVisible` on a column the window had unmounted scrolled to the document origin instead of the column (#2381).
...
| Last backup folder   | UserDefaults     | `PreferenceKeys.lastBackupDirectory` (one path, app-wide; device-local). Read by the Backup Dump sheet, which falls back to Downloads when the path no longer names a folder. |
...
1. **CHANGELOG.md**: Follow [Keep a Changelog 1.1.0](https://keepachangelog.com/en/1.1.0/). Update under `[Unreleased]` using the canonical sections: `Added`, `Changed`, `Deprecated`, `Removed`, `Fixed`, `Security`. Each section appears **at most once per version, in that order**; a second `### Security` under one version is a defect, not a second topic. Do **not** add a "Fixed" entry for fixing something that is itself still unreleased; fold the fix into the Added or Changed entry instead. Documentation-only changes (`docs/`, `CLAUDE.md`, `CHANGELOG.md` formatting) do **not** need a CHANGELOG entry.
    ...
    **Entry shape**: write the entries Keep a Changelog itself writes. An entry is **a fragment naming the change, not a sentence explaining it**: one line, no verb needed, aim under 120 characters. Do not describe the repair, the mechanism, or the before-and-after; those belong in the PR body, where a reader who wants them will look.

    - **Added** and **Changed** take a noun phrase for the thing that now exists: `Open in Window for row inspector text fields.`
