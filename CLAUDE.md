...
**Browsing is per window; a connection owns only its driver's position**: one connection can have any number of windows open on any number of its databases at once, and each of those windows shows exactly one. `MainContentCoordinator.browseState` (`WindowBrowseState`) is that per-window cursor, read through `browseDatabaseName` / `browseSchemaName` / `browseScope` and written only through `applyBrowseState`. `ConnectionSession.driverDatabase` / `driverSchema` / `driverScope` mean something narrower: where the one shared driver is pinned and what a reconnect restores. Use the driver position only for work with no window to ask (reconnect, health checks, MCP); never to seed a tab or drive a sidebar. Three rules follow. First, any cache holding per-container content is keyed by `DatabaseScope`, not by connection id: `SchemaService`, `SchemaProviderRegistry`, `DatabaseTreeMetadataService` and `MetadataConnectionPool` all are, while teardown stays connection-wide (`invalidate(connectionId:)` clears every scope, because a disconnect takes them all). Second, nothing may resolve a browse cursor implicitly from a connection id: `withMetadataDriver` and `SchemaRefreshService` take an explicit scope so a metadata read cannot land on another window's database. Third, `ConnectionToolbarState.syncFromSession` must never resolve `currentDatabase` / `currentSchema`, because a switch fires `connectionStatusChanged` and every window answers it, so reading the shared session there repaints every window's chip with whichever database switched last. Storing the cursor on `ConnectionSession` shipped as two databases of one connection sharing a sidebar, a rail highlight and an autocomplete provider, with Cmd+T opening on the wrong one (#2088), and per-connection restore state brought it back on every relaunch. Engines that cannot hold two containers at once (`requiresReconnectForDatabaseSwitch` and no pooling, e.g. PGlite) legitimately keep one shared cursor, gated on those two capability flags.
