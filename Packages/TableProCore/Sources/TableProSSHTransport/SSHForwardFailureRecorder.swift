...
public final class SSHForwardFailureRecorder: Sendable {
    private let pending = OSAllocatedUnfairLock<SSHForwardFailure?>(initialState: nil)

    public init() {}

    public func record(_ outcome: ChannelOpenOutcome, destination: SSHForwardDestination, deadlineSeconds: Int) {
        guard let failure = outcome.forwardFailure(destination: destination, deadlineSeconds: deadlineSeconds) else {
            return
        }
        pending.withLock { $0 = failure }
    }
    ...
    public func consume() -> SSHForwardFailure? {
