...
enum Td2Token {
    ...
    static func parseServerParams(_ token: [UInt8]) throws -> ServerParams {
        guard token.count >= keyDataOffset else { throw WireError.truncated("server TD2 token") }
        let version = token[0]
        let msgType = token[1]
        guard version == 1 || version == 3 else { throw WireError.malformed("TD2 version \(version)") }
        guard msgType == 1 || msgType == 2 else { throw WireError.malformed("TD2 msgType \(msgType)") }
        let peerCapabilities = be32(token, 8)
        guard token[16] >= 6 else { throw WireError.malformed("TD2 lib version \(token[16])") }
        ...
        func slice(_ length: Int, _ what: String) throws -> [UInt8] {
            guard cursor + length <= token.count else { throw WireError.truncated("TD2 \(what)") }
            defer { cursor += length }
