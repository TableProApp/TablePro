...
struct MSSQLKerberosSPNTests {
    ...
    func buildsRealmQualifiedSPN() {
        #expect(
            MSSQLKerberosSPN.build(host: "sql.example.com", port: 1_433, realm: "resource.realm.com")
    ...
    func nilRealmReturnsNil() {
        #expect(MSSQLKerberosSPN.build(host: "sql.example.com", port: 1_433, realm: nil) == nil)
    }

    @Test("Empty or whitespace-only realm yields no SPN")
    func emptyRealmReturnsNil() {
        #expect(MSSQLKerberosSPN.build(host: "sql.example.com", port: 1_433, realm: "") == nil)
        #expect(MSSQLKerberosSPN.build(host: "sql.example.com", port: 1_433, realm: "   ") == nil)
    }
    ...
    func includesNonDefaultPort() {
        #expect(
            MSSQLKerberosSPN.build(host: "sql.example.com", port: 14_330, realm: "R.COM")
