...
internal func sqliteDefaultValueFromCatalog(_ value: String?) -> String? {
    ...
    if upper == "NULL" || upper == "TRUE" || upper == "FALSE"
        ...
        || sqliteWholeStringLiteral(value) != nil {
        return value
    }
    return sqliteLooksLikeExpression(value) ? "(\(value))" : "'\(sqliteDefaultLiteralEscape(value))'"
}

/// Whether the fragment is one whole single-quoted literal rather than an expression that happens
/// to start with a quote.
internal func sqliteWholeStringLiteral(_ value: String) -> String? {
...
private let sqliteExpressionCharacters: Set<Character> = ["(", ")", "|", "+", "-", "*", "/", "%", "<", ">", "=", "!", "~", "&", " ", ","]

internal func sqliteLooksLikeExpression(_ value: String) -> Bool {
    value.contains { sqliteExpressionCharacters.contains($0) }
}

internal func sqliteDefaultLiteralEscape(_ value: String) -> String {
