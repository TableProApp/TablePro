...
final class ChatToolRegistry {
    ...
    /// The names the app ships. A later `register` cannot take one of them.
    ...
    /// Claims a name for a tool the app ships.
    ...
    /// registry could replace `execute_query` with its own implementation and keep the name every
    /// approval rule is written against.
    @discardableResult
    ...
    /// makes every one of them wait for a human whatever its declared mode says.
