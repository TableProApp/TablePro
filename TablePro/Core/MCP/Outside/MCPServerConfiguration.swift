...
internal enum MCPServerConfigurationValidator {
    ...
    /// the results the assistant hands the server on the wire in the clear; loopback never reaches
    /// one.
    ///
    /// An address carrying userinfo is refused outright. `https://user:secret@host/mcp` would put a
    /// password in the configuration file, draw it in the settings list and hand it back to the
    /// sheet's Address field, which is the whole arrangement the Keychain exists to avoid.
    /// `PairingRedirectValidator` refuses the same shape.
    internal static func validate(name: String, endpoint: URL?) -> MCPServerConfigurationError? {
        ...
        guard endpoint.user == nil, endpoint.password == nil else { return .credentialsInEndpoint }
