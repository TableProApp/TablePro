...
public struct InitializeHandler: MCPMethodHandler {
    ...
    public func handle(params: JsonValue?, context: MCPRequestContext) async throws -> JsonRpcMessage {
        let protocolVersion = params?["protocolVersion"]?.stringValue ?? Self.supportedProtocolVersion
        ...
        Self.logger.info("Initialize: client=\(clientName, privacy: .public) version=\(clientVersion ?? "-", privacy: .public)")
