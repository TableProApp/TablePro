...
extension AIChatViewModel {
    ...
    func ensureColumnsLoaded(forTable tableName: String) async {
        ...
        let task: Task<Void, Never> = Task { [weak self] in
            ...
            do {
                columns = try await DatabaseManager.shared.withDriverScopedMetadataDriver(connectionId: connId) { driver in
            ...
            do {
                fkMap = try await DatabaseManager.shared.withDriverScopedMetadataDriver(connectionId: connId) { driver in
    ...
    private func runSchemaLoad() async {
        ...
        await withTaskGroup(of: (String, [ColumnInfo]).self) { group in
            for table in tablesToFetch where (columnsByTable[table.name] ?? []).isEmpty {
                ...
                group.addTask {
                    do {
                        let cols = try await DatabaseManager.shared.withDriverScopedMetadataDriver(connectionId: connId, workload: .bulk) { driver in
        ...
        do {
            let fkMap = try await DatabaseManager.shared.withDriverScopedMetadataDriver(connectionId: connId, workload: .bulk) { driver in
