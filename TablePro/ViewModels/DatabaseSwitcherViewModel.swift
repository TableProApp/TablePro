...
final class DatabaseSwitcherViewModel {
    ...
    func fetchDatabases() async {
        ...
        do {
            ...
            let names = try await services.databaseManager.withDriverScopedMetadataDriver(connectionId: connectionId) { driver in
                switch target {
            ...
            do {
                let metadataList = try await services.databaseManager.withDriverScopedMetadataDriver(connectionId: connectionId, workload: .bulk) { driver in
