...
struct IOSConnectionExportServiceTests {
    ...
    private func exportedText(includeCredentials: Bool) throws -> String {
    ...
    func plainExportOmitsKey() throws {
        let text = try exportedText(includeCredentials: false)
        #expect(text.contains("bastion.example.com"))
        #expect(!text.contains(keyMarker))
        #expect(!text.contains("db-secret"))
    }

    @Test("An unencrypted export with credentials carries the password but never the private key")
    func credentialExportOmitsKey() throws {
        let text = try exportedText(includeCredentials: true)
        #expect(text.contains("db-secret"))
        #expect(!text.contains(keyMarker))
    }
}
