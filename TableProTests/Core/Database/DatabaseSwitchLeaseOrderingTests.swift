...
struct DatabaseSwitchLeaseOrderingTests {
    ...
    /// registered, so every switch reaches the reconnect and fails there.
    private func registerTypeIfNeeded() {
        ...
        let snapshot = PluginMetadataSnapshot(
            ...
            isDownloadable: false, primaryUrlScheme: "leaseorderingfake", parameterStyle: .questionMark,
            navigationModel: .standard, explainVariants: [], pathFieldRole: .database,
            supportsHealthMonitor: false, urlSchemes: ["leaseorderingfake"], postConnectActions: [],
