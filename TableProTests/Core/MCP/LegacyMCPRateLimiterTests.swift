...
struct MCPRateLimiterTests {
    private func makeLimiter() -> LegacyMCPRateLimiter {
        LegacyMCPRateLimiter()
    }

    private func expectAllowed(_ result: LegacyMCPRateLimiter.AuthRateResult, message: String = "") {
    ...
    @discardableResult
    private func expectRateLimited(_ result: LegacyMCPRateLimiter.AuthRateResult, message: String = "") -> Duration? {
