...
struct SQLStatementPLSQLSplittingTests {
    ...
    func corpusSplitsAsMeasured(example: PLSQLScriptCase) {
        #expect(SQLStatementScanner.allStatements(in: example.script, dialect: .oracle) == example.statements)
    }
    ...
    func caretAnywhereInTheBlockRunsTheBlock() {
        ...
        for caret in 0...blockLength {
            let statement = SQLStatementScanner.statementAtCursor(in: script, cursorPosition: caret, dialect: .oracle)
    ...
    func navigableStatementsSkipSlashLines() {
        ...
        let navigable = SQLStatementScanner.navigableStatements(in: script, dialect: .oracle)
    ...
    func segmentsTileTheDocument() {
        for example in PLSQLScriptCorpus.cases {
            let located = SQLStatementScanner.locatedStatements(in: example.script, dialect: .oracle)
    ...
    func bindParametersFollowTheStatementKind() {
        ...
        """
        let statements = SQLStatementScanner.executableStatements(in: script, dialect: .oracle)
    ...
    func otherDialectsAreUnchanged(dialect: SqlDialect) {
    ...
    func slashLineIsOracleOnly() {
        ...
        #expect(SQLStatementScanner.allStatements(in: script, dialect: .oracle) == ["SELECT 10", "2 AS v FROM dual"])
        #expect(SQLStatementScanner.allStatements(in: script, dialect: .generic) == ["SELECT 10\n/\n2 AS v FROM dual"])
    }
    ...
    func executableTextKeepsTheUnitTerminator() {
        #expect(SQLStatementScanner.executableText(of: "BEGIN NULL; END;\n/\n", dialect: .oracle) == "BEGIN NULL; END;")
        #expect(SQLStatementScanner.executableText(of: "SELECT 1 FROM dual; ;", dialect: .oracle) == "SELECT 1 FROM dual")
        #expect(SQLStatementScanner.executableText(of: "/\n", dialect: .oracle).isEmpty)
    }
    ...
    func caretOnSlashLineRunsTheStatementAbove() {
        ...
        #expect(afterSlashes.map { SQLStatementScanner.statementAtCursor(in: script, cursorPosition: $0, dialect: .oracle) }
