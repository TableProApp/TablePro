...
/// character was thrown away. `settle` is the shape that catches it: drive the pair until nothing
/// moves, and fail if that takes more than a round.
@MainActor
...
struct JsonFieldEditingModelTests {
    ...
    /// Runs the editor and the store against each other the way SwiftUI does: the editor publishes,
    /// the store restates, the editor is told what the store now holds, and round again. Returns
    /// how many rounds it took to stop moving.
    @discardableResult
    private static func settle(
        _ model: inout JsonFieldEditingModel,
        against state: MultiRowEditState,
        limit: Int = 8
    ) -> Int {
        for round in 1...limit {
            let delivered = state.currentText(at: 0)
            guard model.received(delivered) else { return round }
            guard let published = model.typed(model.displayText) else { return round }
            state.updateField(at: 0, value: published)
        }
        Issue.record("The editor and the store never agreed within \(limit) rounds")
        return limit
    }

    @Test("A keystroke survives the round trip and settles in one round")
    func keystrokeSettles() {
        ...
        let rounds = Self.settle(&model, against: state)
        #expect(rounds == 1)
    ...
    @Test("A transiently invalid document settles at every step")
    func invalidIntermediateSettles() {
        ...
        for typed in ["{\"a\": 1,", "{\"a\": 1, \"", "{\"a\": 1, \"b\"", "{\"a\": 1, \"b\": 2}"] {
            ...
            let rounds = Self.settle(&model, against: state)
            #expect(rounds == 1, "stalled on \(typed)")
    ...
    @Test("Editing back to the stored value settles rather than bouncing")
    ...
    @Test("A value the editor did not write is adopted")
