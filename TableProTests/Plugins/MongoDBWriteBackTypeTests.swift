...
struct MongoDBWriteBackTypeTests {
    ...
    func decimalColumnKeepsItsType() throws {
        let stmt = try #require(try update(
            column: "price",
    ...
    func int64ColumnKeepsItsWidth() throws {
        let stmt = try #require(try update(column: "qty", to: .text("5"), kinds: ["qty": .int64]))
    ...
    func doubleColumnKeepsItsType() throws {
        let stmt = try #require(try update(column: "rate", to: .text("3"), kinds: ["rate": .double]))
    ...
    func unknownColumnWritesBareNumber() throws {
        let stmt = try #require(try update(column: "count", to: .text("7"), kinds: [:]))
    ...
    func truncatedValueIsNotWritten() {
        ...
        #expect(throws: MongoDBWriteRefusal.truncatedValue(field: "payload").refusal(ofRow: 0)) {
            try update(column: "payload", to: .text(fragment), kinds: [:])
        }
    }

    @Test("A complete nested value still writes")
    func completeNestedValueWrites() throws {
        let stmt = try #require(try update(column: "payload", to: .text(#"{"a":1}"#), kinds: [:]))
    ...
    func wideDecimalKeepsItsType() throws {
        ...
        let stmt = try #require(try update(column: "price", to: .text(digits), kinds: ["price": .decimal128]))
    ...
    func userTypedBraceStillWrites() throws {
        let stmt = try #require(try update(column: "note", to: .text("{draft"), kinds: [:]))
    ...
    func dateColumnWritesADate() throws {
        let stmt = try #require(try update(column: "when", to: .text("2024-01-02T03:04:05Z"), kinds: ["when": .date]))
    ...
    func pickedDateIsLocalWallClock() throws {
        let stmt = try #require(try update(column: "when", to: .text("2024-01-02 03:04:05"), kinds: ["when": .date]))
    ...
    func dateOnlyIsADate() throws {
        let stmt = try #require(try update(column: "day", to: .text("2024-01-02"), kinds: ["day": .date]))
    ...
    func numberInADateColumnStaysANumber() throws {
        let stmt = try #require(try update(column: "day", to: .text("2024"), kinds: ["day": .date]))
    ...
    func declaredStringIsAlwaysAString() throws {
        for text in ["123", "true", "null", "[1, 2]"] {
            let stmt = try #require(try update(column: "code", to: .text(text), kinds: [:], declared: ["code": .string]))
    ...
    func sampledStringKeepsItsSpelling() throws {
        let stmt = try #require(try update(column: "meta", to: .text(#"{"a":1}"#), kinds: ["meta": .string]))
    ...
    func int32ColumnWritesABareNumber() throws {
        let stmt = try #require(try update(column: "n", to: .text("42"), kinds: [:], declared: ["n": .int32]))
    ...
    func hugeIntegerIsNotRounded() throws {
        let stmt = try #require(try update(column: "big", to: .text("9007199254740993"), kinds: [:]))
    ...
    func exactIntegerStaysBare() throws {
        let stmt = try #require(try update(column: "n", to: .text("9007199254740992"), kinds: [:]))
    ...
    func arrayShapedCodeIsAString() throws {
        ...
        let stmt = try #require(try update(column: "note", to: .text(hostile), kinds: [:]))
    ...
    func quoteInsideAGraphemeClusterIsEscaped() throws {
        ...
        let stmt = try #require(try update(column: "note", to: .text(hostile), kinds: [:], declared: ["note": .string]))
    ...
    func stringIdIsMatchedAsAString() throws {
        let stmt = try #require(try updateFilter(id: "1001", idKind: .string))
    ...
    func hexStringIdIsMatchedAsAString() throws {
        let stmt = try #require(try updateFilter(id: "65a1b2c3d4e5f60718293a4b", idKind: .string))
        #expect(stmt.contains(#"{"_id": "65a1b2c3d4e5f60718293a4b"}"#))
    }

    @Test("A 64-bit _id is matched as one")
    func longIdIsMatchedAsALong() throws {
        let stmt = try #require(try updateFilter(id: "42", idKind: .int64))
    ...
    func mixedIdKindsFallBackToTheSpelling() throws {
        let stmt = try #require(try updateFilter(
    ...
    func int32IsCanonicalised() throws {
        let stmt = try #require(try update(column: "n", to: .text("010"), kinds: [:], declared: ["n": .int32]))
        #expect(stmt.contains(#""n": 10"#))
        let negative = try #require(try update(column: "n", to: .text("-010"), kinds: ["n": .int32]))
    ...
    func unknownIdFallsBackToItsSpelling() throws {
        let stmt = try #require(try updateFilter(id: "65a1b2c3d4e5f60718293a4b", idKind: nil))
        #expect(stmt.contains(#"{"_id": {"$oid": "65a1b2c3d4e5f60718293a4b"}}"#))
    }
}
