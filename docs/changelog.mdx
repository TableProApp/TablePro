...
<Update label="August 19, 2026" description="v0.66.0">
  ### New Features

  - **Tree sidebar layout**: Groups each database or schema's tables, views, materialized views, foreign tables, procedures and functions into collapsible folders (#1590)
  ...
  - **Query Insights**: A Starter feature in **Database > Query Insights** that summarizes the query history on your Mac: which queries you run most, which are slowest, which got slower and which fail. Queries that differ only by their values count as one, and nothing leaves the Mac (#2107)
  ...
  - On connections with schemas, the toolbar shows the database and the schema side by side, each opening its own list, and the schema picker is back at the foot of the object list (#2196)
  ...
  - The flat sidebar keeps its Materialized Views, Foreign Tables, Procedures and Functions sections while a connection reconnects after a dropped SSH or proxy tunnel (#2173)
  - `Cmd+C` in the query editor with nothing selected copies the current line instead of emptying the clipboard, and paste accepts RTF and file-path clipboards (#2172)
  - Sixteen-byte SQL binary columns respect their Display As setting in the grid, the row inspector, VoiceOver and single-cell copy, and keep it across a refresh (#2157)
  ...
  - Query results are editable again when the `SELECT` aliases its table, when the query spans several lines, starts with a comment, or ends in `FOR UPDATE` (#2150)
  ...
  - Content stays readable on the blue selection fill in the connection switcher, the database switcher and the query history drawer (#2140)
...
<Update label="August 16, 2026" description="v0.65.0">
  ### New Features
  ...
  - **PostgreSQL array editor**: Array columns of a simple type, including enum arrays, get a list editor in the grid with one row per element, reordering, and NULL per element (#2095)
  - **MongoDB autocomplete**: `db.` lists collections, `db.users.` lists driver methods, and inside a query you get field names, nested paths and the operators valid in that spot (#2095)
  - **PostgreSQL autocomplete**: Enum values are suggested when you compare against an enum column, alongside about 400 built-in functions, the operator set, and multi-word syntax such as `ON CONFLICT DO UPDATE SET` (#2095)
  - **Syntax problems as you type**: The editor underlines structural mistakes such as an unclosed bracket or comment. A half-written statement is never flagged (#2095)
  - **Legacy UUID Encoding for MongoDB**: Binary UUIDs written by the Java, C# or Python drivers read as UUIDs instead of hex, and filters, edits and exports write the same bytes back (#2086)
  ...
  - **SSL settings on mobile**: A mode picker plus CA, client certificate and client key for MySQL, PostgreSQL and Redis, from a PEM file, pasted text, or PKCS#12 (#2083)
  ...
  - A query that fails shows the database's error again and is recorded in history. A syntax error left the results area blank with no message at all (#2120)
  ...
  - The drop, truncate, delete and external-link confirmations are standard alerts that default to Cancel, so `Return` no longer drops a table (#2104)
  ...
  - Tables you create in an in-memory DuckDB database show up in the sidebar (#2108)
  - Editing a connection on mobile no longer wipes its SSL settings and per-database options (#2083)
...
<Update label="August 10, 2026" description="v0.64.0">
  ### New Features

  - **Workspace rail**: A rail on the leading edge of the sidebar lists every connection and database you have open, so one click switches to it. It has its own shortcuts and keyboard control (#1282)
  - **Match Case**: Filter operators can match case on every database that can express it, including MongoDB, Elasticsearch, DynamoDB, and etcd (#2048)
  ...
  - **Oracle administrative logons**: Connect as SYSDBA or SYSOPER, on Mac and mobile (#2039)
  - **Oracle on TablePro Mobile**: Browse schemas and tables, run queries, and edit rows, with a Service Name or SID picker (#2033)
  ...
  - MCP and AI table tools take a `database` argument, so a table in another database can be inspected without changing the app (#2026)
  - The "Group all connections in one window" setting is gone. Each connection has its own window and tab bar (#1282)

  ### Bug Fixes

  - A tab keeps running against the database it was opened on, so changing database no longer breaks it with a "table doesn't exist" error or writes to the wrong place (#2026)
  ...
  - Clicking through the sidebar no longer keeps loading tables you have left, or shows the previous table's rows (#2058)
  ...
  - Oracle connections turn on TCP keepalive, so an idle session is less likely to be dropped (#2038)
...
<Update label="August 5, 2026" description="v0.63.0">
  ...
  - `Cmd+Delete` and `Option+Delete` in the SQL editor delete to the start of the line and the previous word again (#2022)
  - Refreshing a table no longer fails with "Query cancelled" on every second click, and stopping a query no longer shows a red error, records a failure in history, or cancels the next query you run on MySQL, MariaDB, and Redis (#2021)
  - Generated columns: SQL export leaves them out of the INSERT statements so the dump imports cleanly, adding or duplicating a row saves, and the data grid shows them as read-only. Covers MySQL, MariaDB, and ClickHouse (#2023)
...
<Update label="August 2, 2026" description="v0.62.0">
  ...
  - **On Update column**: Set a MySQL or MariaDB timestamp column to update itself from the Structure tab, without writing the SQL by hand (#2005)
  ...
  - SQL editor tabs are saved about a second after you stop typing instead of waiting up to 30 seconds (#1997)
  ...
  - Opening a SQL Server table or view outside the default schema no longer fails with "Invalid object name", and switching database no longer leaves the sidebar and the query on different schemas (#2004)
  ...
  - A MySQL or MariaDB timestamp column that keeps fractional seconds saves again (#2005)
  - Quitting no longer loses unsaved SQL editor tabs. A window still waiting on its connection could erase the saved tabs for it, and a query over 500KB is now kept in full rather than restored empty (#1997)
...
<Update label="July 30, 2026" description="v0.61.0">
  ### New Features

  - **Connect from a project folder**: Pick a folder from the welcome screen or File > Open Project Folder, and TablePro reads the database settings it finds in `.env`, `wp-config.php`, `prisma/schema.prisma`, `config/database.yml`, `docker-compose.yml`, `application.properties`, `application.yml`, or `appsettings.json`. Review what it found and the connection form opens filled in (#1959)
  - **AI walkthroughs**: Explain, Optimize, and Fix Error now answer with numbered steps and a before/after SQL diff you can apply to the editor (#1945)
  - **Close tabs in groups**: Close All Tabs, Close Other Tabs, and Close Tabs for Other Databases in the File menu. None has a shortcut out of the box; bind them in Settings > Keyboard (#1972)
  ...
  - **AI tool call limit**: A limit you can raise or turn off in Settings > AI, now 25 per reply instead of a fixed 10. Reaching it pauses the reply and keeps what the AI found, with Continue to carry on (#1987)
  - **Pin button on result tabs**: Pinning was already there, but only in a right-click menu and the View menu (#1982)
  ...
  - An SSH tunnel that cannot reach the database reports whether the connection was refused or timed out, instead of leaving the driver to report a handshake timeout. The usual cause is Host holding the server's public address when the database only listens on `127.0.0.1` (#1981)
  ...
  - iCloud Sync now sends only the fields that changed, so editing a connection on two devices no longer overwrites the field the other one changed. It also reports a failed sync instead of always reporting success, downloads every page when catching up, and retries the changes iCloud rejected instead of dropping them (#643)
  - Connection changes made on the Mac reach the iPhone and iPad app again, and a synced connection keeps its safe mode setting and its colour tag (#643)
  ...
  - A PostgreSQL partitioned table is listed once, with its partitions under it, so hundreds of partitions no longer bury the rest of the schema or fill up autocomplete (#1984)
  ...
  - MongoDB authenticates against the database set on the connection, not the one you are browsing, and creating a database now asks for its first collection so it is kept (#1970)
  - Exporting a MongoDB collection works again, includes fields that first appear after the opening document, and `db.getCollection("name")` is accepted in the query editor (#1971)
  - `postgres`, CockroachDB's `defaultdb`, and Redshift's `dev` are no longer hidden as system databases, and the database a connection is using always shows in the sidebar and switchers (#1967)
  ...
  - Right-clicking below the SQL editor opens the menu for the result tabs, data grid, or status bar instead of the editor's, and View > Pin Result is enabled only when there is a result tab to pin (#1982)
  - Closing a window asks about unsaved changes in every tab, not just the one on screen (#1972)
  - The AI chat panel stays inside the right panel at narrow widths, and a message sent right after launch is no longer replaced by the restored conversation (#1956)
  - BigQuery `REPEATED` columns no longer show every element as `null`, and `STRUCT` cells show nested structs and arrays as JSON instead of escaped text (#1963)
</Update>

<Update label="July 25, 2026" description="v0.60.1">
  ### New Features

  - **RDS Endpoint**: AWS IAM connections have an RDS Endpoint field, for when the connection points at a port forward you run yourself and TablePro cannot tell which database is behind it (#1432)

  ### Improvements

  - The AWS profile field on MySQL and PostgreSQL connections now lists the profiles found on disk, like MariaDB already did (#1432)
...
<Update label="July 24, 2026" description="v0.60.0">
  ### New Features

  - **PGlite**: Connect to PGlite through its socket server (`pglite-server`), with schema browsing, SQL editing, and row editing like PostgreSQL (#1911)
  ...
  - **CSV file properties**: In the CSV editor, set the delimiter, quote character, escape character, encoding, and line ending from Edit > Set CSV Properties…, then Reload to re-read the file. Switch the first row between header and data with `Cmd+Shift+H` (#1469)
  - **Local AI providers**: Add llama.cpp and MLX as local AI providers, each pointing at the server's default local endpoint with no API key (#1777)
  ...
  - Switching schemas reloads the table list once per connection instead of once per window, so the sidebar no longer flashes its loading spinner once per open tab (#1946)
...
<Update label="July 21, 2026" description="v0.59.0">
  ### New Features

  - **Trino**: Trino as a downloadable native Swift driver. Connect with a username and password or a JWT token, optionally over TLS with a custom CA or client certificate, browse catalogs, schemas, tables, and columns, run SQL, edit rows, and create or alter tables (#1906)
  - **SSH auth: None**: SSH tunnels support None as an auth method, for hosts that handle SSH auth themselves like Tailscale SSH (#1907)
  - **SSH keyboard-interactive 2FA**: SSH tunnels support keyboard-interactive verification, so a server that needs a private key plus a one-time code now connects. TablePro shows the prompt and you type the code (#1920)
  ...
  - Copy as INSERT, UPDATE, JSON, CSV, or Markdown copies only the cells you selected, not the whole row. Copy as UPDATE still keys its WHERE clause on the primary key (#1929)
  ...
  - The MongoDB, Oracle, Cassandra, and Elasticsearch plugins install again instead of failing with "Bundle failed to load executable" (#1917)
  - Plugin install and load failures now name the real cause (wrong architecture, missing dependency, or version incompatibility) instead of a generic error (#1915)
</Update>
...
<Update label="July 15, 2026" description="v0.57.1">
  ...
  - **SSH to a Unix socket**: SSH tunnels can forward to a Unix socket on the remote server, for databases that only listen on a socket. Set the socket path on the General pane and TablePro forwards to it through your jump hosts (#1871)

  ### Bug Fixes

  - Fixed a crash that could quit the app when opening or sorting a table whose columns have comments (#1869, #1880)
  - The right sidebar resizes again while a Users & Roles tab is open, and the user list collapses on its own when the tab gets too narrow (#1872)
  - Fixed dividers that could not be dragged at all in Users & Roles, the Structure trigger list, and the Server Dashboard metrics and slow query panes (#1872)
  - Cancel now stops a connection attempt right away, drops the cancelled connection from the last session, and no longer interrupts a later successful connection (#1358)
  - MCP tools keep working after the server sits idle for 15 minutes or is restarted from Settings, so agents like Claude Code and Cursor no longer need to be turned off and on again (#1881)
...
<Update label="July 14, 2026" description="v0.57.0">
  ### New Features

  - **SurrealDB**: SurrealDB 2.x and 3.x as a downloadable driver. Browse namespaces and databases, run SurrealQL, and edit records in the grid (#1862)
  - **Beancount**: Beancount ledgers as a downloadable, read-only driver. Transactions, postings, accounts, prices, and balances read as SQL tables, and BQL runs with a `BQL:` prefix (#1474)
  - **Users & Roles**: Manage database users, roles, and privileges on MySQL and PostgreSQL from View > Users & Roles. Pick any object from the server down to a single column, see where each privilege comes from, and grant or revoke it. Changes are staged, undoable, and shown as SQL before they run (#1413)
  - **Cloud SQL Auth Proxy**: Connect to Google Cloud SQL without starting the proxy yourself. Turn it on for a MySQL, PostgreSQL, or SQL Server connection, set the instance connection name, and TablePro starts and stops the proxy for you (#1728)
  ...
  - **Reopen Closed Tab**: `Cmd+Shift+T` brings back the tab you just closed, and File > Recently Closed lists the last 20, kept for 30 days (#1854)
  - **Open from the terminal**: Install the `tablepro` command from Settings > General, then run `ddev tablepro` in a DDEV project. Links to a database on your own Mac can be trusted with Always Allow and reviewed in Settings > General (#1486)
...
<Update label="July 10, 2026" description="v0.56.2">
  ### New Features

  - **Resizable JSON field**: Drag the handle below a JSON value in the row details inspector to make it taller and read more in place. The height is remembered across rows and app restarts (#1849)

  ### Improvements

  - Column comments now appear directly in data grid column headers when object comments are enabled, instead of only in the header tooltip (#1789)
...
<Update label="July 9, 2026" description="v0.56.1">
  ### Bug Fixes

  - Oracle now connects to servers that require native network encryption (`SQLNET.ENCRYPTION_SERVER = REQUIRED`), negotiating AES with a SHA checksum automatically like SQL Developer and DBeaver. The separate encryption option is gone; encryption is offered to every server and used only when the server asks for it (#483)
  - Reading a connection password from a command, 1Password, Vault, or AWS Secrets Manager no longer occasionally returns corrupted output from a race while reading the output (#1841)
  - MongoDB filters and generated edit statements now quote numeric-looking values that would not round-trip, such as `.5`, `1.`, `+7`, `01`, and very large integers, instead of emitting invalid or lossy JSON. Update the MongoDB plugin to get the fix (#1813)
  - The Structure tab filter and column sort now update the grid instead of leaving stale rows on screen
  - The row details inspector now shows the selected row's values, including JSON, when a column value filter is active, and an open value follows the selected row as you move between rows (#1837)
...
<Update label="July 9, 2026" description="v0.56.0">
  ### New Features
  ...
  - **Recent tables**: A Recent section at the top of the sidebar lists the last 10 tables you opened per connection and database, and remembers them between launches. Off by default; turn it on in Settings > General > Sidebar (#1352)
  - **Delete confirmation**: Saving now asks you to confirm before it permanently deletes rows (#1823)

  ### Improvements

  - The sidebar filter matches table and view names by substring instead of fuzzy matching, and sorts names that start with what you typed to the top (#1822)

  ### Bug Fixes

  - Typing in the SQL editor no longer crashes the app, and the autocomplete popup appears again as you type (#1835)
  - Deleting many rows at once no longer freezes the app; multi-row deletes run as one batched statement instead of one query per row (#1823)
  ...
  - Sorting a table column keeps your rows-per-page setting and returns to page 1 instead of loading the whole table (#1826)
  ...
  - Switching schemas on an Oracle connection no longer hangs on an infinite spinner. Oracle switches by schema, loads each schema's tables on expand, respects the query timeout with automatic reconnect, and shows an error with a Retry button when a schema fails to load (#1807)
  ...
  - Hidden columns stay hidden, and resizing a column no longer brings them back. Column widths, order, and visibility are remembered per table, kept separately for each connection, database, and schema. A new Reset Columns button in the Columns popover restores defaults (#1815)
  - Query and filter errors now appear in a banner you can select and copy, with a Fix with AI button, instead of a dialog that cut the message off (#1815)
...
<Update label="June 30, 2026" description="v0.54.0">
  ...
  - **Table and column comments**: Comments from the database now show in the sidebar, the column header tooltip, and the table inspector. Toggle from View > Show Object Comments. Available for MySQL and PostgreSQL (#1771)
  ...
  - Safe mode keeps the confirmation on the tab you ran the query from instead of jumping to the first tab (#1781)
  - Switching between sidebar tables no longer leaves blank space above the list (#1675)
  - SSH tunnels no longer pin a CPU core after the connection drops (#1769)
  - Restored table tabs load with the current page size from this session.
  - SQL Server: large nvarchar(max) and text values no longer truncate when copied or viewed (#1783)
