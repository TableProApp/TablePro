...
<Frame caption="Cloudflare Tunnel selected on the Network tab">
  <img className="block dark:hidden" src="/images/cloudflare-tunnel-pane.png" alt="Access hostname, authentication method and local listener fields" />
  <img className="hidden dark:block" src="/images/cloudflare-tunnel-pane-dark.png" alt="Access hostname, authentication method and local listener fields" />
</Frame>
...
<Steps>
  <Step title="Enable the pane">
    On the **Network** tab, set **Connect via** to **Cloudflare Tunnel**. A connection uses one transport, so choosing this one switches off whichever was selected before.
  </Step>
