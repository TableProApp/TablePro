...
| **Tunnel Command** | Hold a `kubectl port-forward`, an AWS SSM session, or a command of your own. See [Tunnel Command](/connections/tunnel-command) |
...
Only one of SSH Tunnel, Cloudflare Tunnel, Cloud SQL Auth Proxy, SOCKS Proxy, and Tunnel Command can be on at a time. Turning on a second offers a button to switch off the first.
