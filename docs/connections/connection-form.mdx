...
description: The four tabs of the connection editor and the fields each driver adds
---

The editor has the same four tabs whatever the driver. Only Network is ever absent, and only for a driver that connects directly and cannot negotiate TLS.

<Frame caption="A PostgreSQL connection on the General tab">
  <img className="block dark:hidden" src="/images/connection-form-fields.png" alt="Connection editor showing Name, Type, host fields and credentials, with a tab bar above and Test, Cancel and Save below" />
  <img className="hidden dark:block" src="/images/connection-form-fields-dark.png" alt="Connection editor showing Name, Type, host fields and credentials, with a tab bar above and Test, Cancel and Save below" />
</Frame>
