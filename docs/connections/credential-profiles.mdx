...
Credential profiles stay on the Mac that created them. Sync for them is not switched on yet: the record type ships ahead of its CloudKit schema, so nothing is sent or received. Create the profile again on a second Mac for now. Connections linked to a profile keep that link when they sync; the link resolves to nothing on a Mac that has no profile with that id, and the connection's **Credentials** picker says so.
