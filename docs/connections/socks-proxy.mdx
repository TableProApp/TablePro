...
Leave the database's **Host** and **Port** on the General tab exactly as they are. The proxy resolves that name and dials it from its own side. A hostname that exists only inside the private network works, and no DNS query for the database leaves your Mac.

<Frame caption="SOCKS Proxy selected on the Network tab">
  <img className="block dark:hidden" src="/images/socks-proxy-pane.png" alt="Proxy server host and port with a username and password below" />
  <img className="hidden dark:block" src="/images/socks-proxy-pane-dark.png" alt="Proxy server host and port with a username and password below" />
</Frame>
