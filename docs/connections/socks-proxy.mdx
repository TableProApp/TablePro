...
Bytes through the proxy are counted the same way an SSH tunnel's are, and the totals and the live rate sit under the connection switcher. See [transport activity](/connections#switch-connections-and-databases).
