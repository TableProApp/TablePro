...
When the only way out of a corporate network is a SOCKS5 endpoint, the database's **Host** and **Port** on the General pane stay exactly as they are: the proxy resolves that name and dials it from its own side. A hostname that exists only inside the private network works, and no DNS query for the database leaves your Mac.
