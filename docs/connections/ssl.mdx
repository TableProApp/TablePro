...
A new connection starts on the mode that matches the driver's own default, and the driver's own guidance prints under the picker where there is any.
...
| etcd | Disabled | Nothing. The driver never reads these fields. Set **TLS Mode** on the Options tab instead, and see [etcd](/databases/etcd) |
...
| Snowflake, BigQuery, DynamoDB, Cloudflare D1, libSQL / Turso | Always encrypted | These drivers are HTTPS and manage TLS themselves. No SSL/TLS section |
