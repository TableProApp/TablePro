...
<Frame caption="The kubectl preset, with the argument list it will run underneath it">
  <img className="block dark:hidden" src="/images/tunnel-command-pane.png" alt="Tunnel Command selected, with the kubectl method, namespace, resource and a Will Run section" />
  <img className="hidden dark:block" src="/images/tunnel-command-pane-dark.png" alt="Tunnel Command selected, with the kubectl method, namespace, resource and a Will Run section" />
</Frame>
...
<Steps>
  <Step title="Choose the transport">
    On the **Network** tab, set **Connect via** to **Tunnel Command**. A connection uses one transport, so choosing this one switches off whichever was selected before.
  </Step>
...
The **Host** and **Port** on the General tab stay the database's own. They are what the forward points at, so a kubectl forward reads the port from there and an SSM session forwards to that host and port from the target instance.
...
| `{host}` | The **Host** field on the General tab |
| `{remotePort}` | The **Port** field on the General tab |
