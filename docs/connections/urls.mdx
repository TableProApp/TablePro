...
Two readers take these URLs and they do not take the same set: the **Import from URL…** sheet accepts every scheme in this reference, while `open` and browser links reach TablePro only through the ones it registers with macOS.
