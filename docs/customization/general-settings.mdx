...
The update window shows the release highlights: the handful of lines a release opens with. Where a release has none, it shows the whole changelog for that version, new features first. Either way the full changelog is one click away.

**What's New** opens a window with the highlights of the version you are running, and a button to the full changelog. It is also available from **Help > What's New**.

**Download and install updates automatically** is on. A new version downloads in the background and installs the next time TablePro quits, with no dialog. Turn it off to be asked before anything installs.

**Automatically check for updates** is on and checks in the background. **Check for updates** sets how often: **Daily** or **Weekly**. Weekly means a security fix can arrive up to a week later.

**Check for Updates…** checks now, here or as **TablePro > Check for Updates…** in the menu bar. It ignores both the schedule and any staged rollout, so it always offers the newest build. Updates come through [Sparkle](https://sparkle-project.org/) and are signature-checked before they install.

An administrator can pin all three across a fleet. See [Managed updates](/security/managed-updates).
