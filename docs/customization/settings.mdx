...
The **Integrations** tab runs the MCP server. **Enable MCP Server** (default off) starts it and shows a live status. See [MCP Server](/features/mcp) for a walkthrough and the [External API](/external-api/index) reference for the tool catalog. The server binds `127.0.0.1` only.
