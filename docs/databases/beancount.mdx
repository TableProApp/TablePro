...
Install `rledger` or Python Beancount before you connect. The plugin shells out to one of those two and projects the answer into 18 read-only SQL tables. Every projected directive comes from the backend's own output; the plugin reads the ledger text only to follow `include` directives and document roots.
...
The backends disagree about generated entries. For the transaction a `pad` inserts, `rledger` leaves these columns empty and Python Beancount points them at the `pad`. A `pad` a Beancount plugin generated keeps whatever name its metadata carries in `source_file`, and projects with `line` and `source_location` null.

`rledger` exposes no pad accounts as query columns, so on that backend `pads.account` and `pads.source_account` come from `PRINT`, which renders each directive from the parsed ledger. Verified against rustledger 0.22.0.
...
Rows from `rledger check` carry `phase` `validate`. Rows the projection wrote itself carry `projection`, and cover any pad directive whose accounts it could not resolve.
