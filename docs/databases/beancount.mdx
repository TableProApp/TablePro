...
The plugin also supports BQL queries through `rledger`. Install rustledger separately so the `rledger` executable is available on your `PATH`, or launch TablePro with `TABLEPRO_RUSTLEDGER_BINARY` set to the executable path.

## rledger requirement

Beancount support uses the external `rledger` executable to parse ledger data and run BQL. TablePro does not bundle `rledger`.

```bash
rledger --version
```

If TablePro cannot find `rledger`, set the executable path before launching the app:
