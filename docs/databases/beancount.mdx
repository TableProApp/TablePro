...
Install `rledger` or Python Beancount before you connect. The plugin parses nothing itself: it shells out to one of those two and projects the answer into 17 read-only SQL tables. **Beancount Driver** is a registry plugin, so picking **Beancount** in the connection form prompts to install it too.
