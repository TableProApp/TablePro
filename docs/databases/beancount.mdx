...
The Python backend does not run plugins declared by a ledger unless you explicitly trust that ledger and opt in:

```bash
launchctl setenv TABLEPRO_BEANCOUNT_ALLOW_PYTHON_PLUGINS 1
```

Python plugins are ordinary executable code. Leave this unset for ledgers you did not create or audit. Suppressed plugin names appear in `diagnostics` with phase `security`.
