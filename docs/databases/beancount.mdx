...
Install `rledger` or Python Beancount before you connect. The plugin shells out to one of those two and projects the answer into 19 read-only SQL tables.
...
| `directive_metadata` | User metadata on non-transaction dated directives |
...
`directive_metadata` carries the source location of the metadata line. Transaction and posting metadata stay in their dedicated tables.
...
- Directives outside the 19 tables are not projected. They stay in the source files.
