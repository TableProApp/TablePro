...
- A job is polled until the [query timeout](/customization/settings#query-execution), then cancelled. The floor is 30 seconds, so **No limit** gives BigQuery 30 seconds rather than none.
