...
Every query travels over HTTP to port 8123. The native TCP protocol on 9000 is not used, so the server's HTTP interface has to be on. The driver ships inside the app; there is nothing to install.

## Quick setup

Click **Create Connection…**, select **ClickHouse**, enter host, port, credentials, and database, then click **Save & Connect**.
