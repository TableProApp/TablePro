...
Switching database happens in place, with no reconnect, and the sidebar's choice only affects new tabs: an open tab stays on its own database. See [Tabs](/features/tabs#database-binding). For unencrypted HTTP to a remote server, use [SSH tunneling](/connections/ssh-tunneling).
...
Each request carries a query id and asks for progress in the HTTP headers, so a long query reports rows read as it goes. Cancelling sends `KILL QUERY` for that id, stopping the query on the server rather than only dropping the connection. The [query timeout](/customization/settings#query-execution) bounds each request at the timeout plus 30 seconds.
