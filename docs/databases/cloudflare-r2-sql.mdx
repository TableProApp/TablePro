...
That is the whole form. There is no username, port, SSH Tunnel section, or SSL/TLS section: the API is HTTPS only. The Iceberg warehouse name is derived from the account ID and the bucket, so you never type it.
...
- No SSH tunnel, Cloudflare Tunnel, SOCKS proxy, or SSL/TLS section. The API is HTTPS only.
