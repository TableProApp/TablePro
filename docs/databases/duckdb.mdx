...
DuckDB 1.5.2 ships inside the driver plugin, an embedded analytical engine built for OLAP workloads. Pick DuckDB in the connection form and you are prompted to install the plugin.
...
The sidebar shows both levels. Switch it to Tree layout from View > Sidebar Layout to get database > schema > object folders; in the default Flat layout it lists the current schema's objects and the active schema sits at the foot of that list. The toolbar shows the active database and schema side by side; click either to switch it, or press `Cmd+K` for the database list.
