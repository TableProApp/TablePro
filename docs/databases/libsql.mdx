...
**Connection Mode** decides every other field on the form. Remote speaks the Hrana HTTP protocol to Turso or a self-hosted sqld; Local File opens a libSQL file with the SQLite engine the driver carries.

<RegistryPlugin name="libSQL / Turso" plugin="libSQL / Turso Driver" />

## Quick setup

Click **New Connection…**, select **libSQL / Turso**, pick a **Connection Mode**, fill in the fields it leaves on screen, then click **Test Connection** and **Save & Connect**.
...
The API Token field is hidden here; a local file needs no authentication. **Options** gains an **Extensions** list in this mode, which loads SQLite extensions the way it does for [SQLite](/databases/sqlite#extensions).
...
There is no SSL/TLS section. A Remote connection is encrypted when its **Database URL** starts with `https://` and is not when it starts with `http://`, and a local file is not networked at all. See [SSL/TLS](/connections/ssl).
...
- libSQL-only SQL such as `ALTER TABLE ALTER COLUMN` does not run in Local File mode. The SQLite engine gets the statement and rejects it. Rewrite it as standard SQLite DDL.
- Remote mode loads no extensions. A Turso database runs the extensions its server was built with.
