...
- A binary value keeps the subtype it was read with. New bytes in a field whose validator does not declare `binData`, or the same bytes read with two different subtypes, refuse the save: write that value with a query.
