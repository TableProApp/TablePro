...
| **Port** | `1433` | |
| **Authentication** | `SQL Server Authentication` | Or **Windows Authentication (Kerberos)** or **Microsoft Entra ID**, both macOS only. See [Windows Authentication](#windows-authentication-kerberos) and [Microsoft Entra ID](#microsoft-entra-id) below |
...
Available on macOS. Set **Authentication** to **Microsoft Entra ID** to sign in with your work
account instead of a SQL Server login. This covers Azure SQL Database, Azure SQL Managed Instance,
and SQL Server 2022 with Entra authentication enabled.

Sign-in uses the device code flow, so it honours multifactor authentication and Conditional Access:
...
**Limitations**: Windows Authentication (Kerberos) and Microsoft Entra ID are macOS only, Entra ID signs in with the device code flow only, named instances unsupported (use host and port), Verify CA and Verify Identity behave like Required.
