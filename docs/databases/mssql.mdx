...
**File > Import** runs a `.sql` file the same way, `GO 5` included, and never sends a `GO` line. A failed batch is reported at the line it starts on, and its error names the line in the file: `Failed at line 3. Line 5: Invalid object name 'orders'.` A batch past 64 million characters, about half of what SQL Server takes in one request, ends at its next `;`, so a dump written with no `GO` line still imports.

## SSL/TLS

New connections start on **Preferred**, which encrypts the login and leaves queries and results in plain TCP unless the server forces encryption. Pick **Required (skip verify)** or stricter to encrypt the whole session.

| Mode | Behavior |
|---|---|
| **Disabled** | The login over TLS when the server offers it, then plain TCP. A server that forces encryption gets TLS throughout |
| **Preferred** | Same as Disabled |
| **Required (skip verify)** | TLS throughout, certificate unchecked. A server without TLS is refused |
| **Verify CA** | TLS throughout, certificate chained to a trusted authority. The host name is not checked |
| **Verify Identity** | TLS throughout, chain checked, and the certificate names the host you dialled |
