...
- SQL*Plus commands such as `SET SERVEROUTPUT ON`, `SHOW ERRORS` and `EXEC` are not SQL and fail when sent. Delete them, and write `EXEC p` as `BEGIN p; END;`.
- `DBMS_OUTPUT` lines are not shown. Return the value from a query instead, or write it to a table.
