...
**Schemas**: The sidebar shows all accessible schemas and tables. Cmd+K switches databases; pick the active schema from Database > Schema, and the toolbar shows the active one. Table info shows columns, indexes, constraints, and DDL. Right-click a schema for **Drop Schema...**, which runs `DROP SCHEMA ... CASCADE`, so objects in other schemas that depend on it are dropped too. Select several databases or schemas with Shift-click or Cmd-click and the menu covers all of them. See [Work on Several Databases at Once](/features/table-operations#work-on-several-databases-at-once).
