...
Another process holds a write lock. Quit the app that owns the file, or look for `-wal` and `-shm` files a crashed process left behind. The wait runs until the [query timeout](/customization/settings#general); **Query > Cancel Query** ends it sooner.
