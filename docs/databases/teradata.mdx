...
Teradata negotiates TLS on the same gateway port (`1025`), so there is no separate HTTPS port. Set the SSL Mode to turn it on:

- `Preferred` tries TLS and falls back to a plain connection if the gateway does not offer it.
...
To test TLS against a server that offers it, use `Required` so a failed handshake surfaces instead of falling back.
