...
- `INFORMATION_SCHEMA`, `METRICS_SCHEMA`, `PERFORMANCE_SCHEMA`, `mysql` and `sys` are system databases: the database switcher lists them under **System**, and the sidebar tree shows them only with [**Show system databases and schemas**](/customization/general-settings#tabs-and-sidebar) on, or when one is the database in use. A TiDB server saved as a MySQL or MariaDB connection lists `METRICS_SCHEMA` with your own databases. Sequences are not listed as tables.
