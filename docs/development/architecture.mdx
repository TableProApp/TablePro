...
A driver is one of three roles a bundle can take. `ExportFormatPlugin` and `ImportFormatPlugin` are the others, which is how the CSV, JSON, SQL, XLSX, and MQL formats ship. `DocumentInspectorPlugin` is still declared in the kit, and the app no longer loads a bundle that provides it.
...
|---------|--------|---------|
| TableProEditor | Local package in `Packages/`, a heavily modified fork of CodeEditSourceEditor and CodeEditTextView | The SQL editor: text engine, layout, highlighting, folding |
| TableProGrammars | Local package in `Packages/` | The four tree-sitter grammars the editor highlights |
...
| Sparkle | SPM, 2.10.0+ | Auto-update with EdDSA signing |
