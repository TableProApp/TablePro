...
Lint covers `TablePro/` and `Packages/`, which includes `TableProPluginKit` through the symlink in
`TableProCore`. The rest of `Plugins/` and the app's own test bundles get no automated style check,
so on a change there the conventions on this page are all there is.
