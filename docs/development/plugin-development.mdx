...
|-----|------|----------|---------|
| `TableProPluginKitVersion` | integer | Yes | PluginKit ABI version the plugin was built against (19) |
...
`scripts/build-pluginkit-xcframework.sh` produces a distributable `TableProPluginKit.xcframework` covering arm64 and x86_64. Run it here, or download a published one from the `pluginkit-v<version>` release, and link it from your own Xcode project. Your plugin target then needs nothing from this repository.
