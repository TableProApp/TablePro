---
title: MCP Protocol
description: "Wire contract for TablePro MCP server: protocol versions, required metadata, headers, result envelope, cache hints, and error codes"
---
...
4. A session lives until the server stops. The store holds at most 16 at a time and evicts the least recently used past that. A session the server does not recognize is `-33001`, and the client has to `initialize` again.
