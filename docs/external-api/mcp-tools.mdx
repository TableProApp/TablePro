...
Statements that read or write files, or that run server-side code, are refused on every tool. So is more than one statement in a single call, except a SQL Server script sent to [`execute_query`](#execute_query).
