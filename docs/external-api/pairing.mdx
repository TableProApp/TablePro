...
sequenceDiagram
    ...
    T->>E: open redirect URL with ?code=<one-time-code>
...
TablePro appends `?code=<uuid>` to the `redirect` URL and opens it with `NSWorkspace.shared.open(...)`. Raycast routes the callback to the extension's `pair-callback` handler.

### 6. Extension exchanges the code

The extension reads the MCP port from `~/Library/Application Support/com.TablePro/mcp-handshake.json`, then:
...
| User can revoke any time | **Settings > Integrations > Tokens > Revoke**. |
