...
Run `open -b com.TablePro "mysql://root@127.0.0.1:3306/shop"` from any shell and that database opens in a window. A saved connection is matched if there is one; if not, the session is temporary and nothing lands on disk. `.sql` files and database files take the same route, and DDEV projects get a `ddev tablepro` command of their own.
