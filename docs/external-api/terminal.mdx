...
If `/usr/local/bin` is not writable, Settings shows a `sudo` command you can copy and run instead. TablePro never asks for your admin password. Run the command and switch back to TablePro, and Settings picks up the change on its own.
