...
const params = new URLSearchParams({
  ...
  redirect: "raycast://extensions/datlechin/tablepro/pair-callback",
...
The user does not need to enable MCP in Settings beforehand. The first call starts the server on a free port in the `51000-52000` range and writes a handshake file at `~/Library/Application Support/com.TablePro/mcp-handshake.json`.
...
Creates a saved connection from query parameters and opens the connection editor for review. See [Deep Links](/features/deep-links#import-a-connection) for the full parameter list. This action is unchanged by the External API rework.
