...
The first form opens the table in the connection's current database and schema. The second selects a database first. The third (Postgres-style) selects both, database before schema.

On a connection that has schemas but no databases to switch between, such as Oracle, a lone `database` segment names the schema. A segment naming a dimension the connection does not have is ignored rather than applied to the other one.
