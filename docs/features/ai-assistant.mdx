...
Press `Cmd+Option+A`, or choose **View > Show Assistant**, to open the assistant in the window's trailing pane. It shares that pane with the inspector: opening one puts the other away, and the same shortcut closes it again. The toolbar button ships in the default set, so a toolbar customized before this release needs it added from **View > Customize Toolbar…**.
