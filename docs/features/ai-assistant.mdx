...
Whichever segment you pick is remembered for that connection, and only a segment you pick counts: opening the pane for a row you clicked, with **Auto-show inspector on row select** on, leaves a pane you left on the assistant alone. The sparkles button is no longer in the default toolbar, and [Customize Toolbar](/features/connection-window#add-a-control-back) puts it back.
