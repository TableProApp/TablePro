...
A reply pauses after 25 tool calls, keeps everything it has done, and offers **Continue** for a fresh budget or **Adjust Limit** to change the number, 5 to 200, under **Agent** in [Settings > AI](/customization/settings#agent). Every call is another request with the schema attached, so a higher limit costs tokens.
