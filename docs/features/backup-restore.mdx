...
<Info>
PostgreSQL and Redshift only. Restore is disabled on read-only [Safe Mode](/features/safe-mode). Backup stays available on read-only connections because it does not write to the database.
</Info>
...
Restore progress is indeterminate: the live byte counter runs only during backups, so long restores show activity without a percentage.
...
Both flows reuse the connection's active SSH tunnel. No second port forward is opened. SSL connections pass their mode to the tools via `PGSSLMODE`, including `verify-ca` and `verify-full`.
