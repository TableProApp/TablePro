...
With a row limit, the comparison stops after that many keys in key order. Rows past the limit are not read on either side, so none of them is inserted or deleted.
