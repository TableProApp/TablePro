...
A column's type is compared as the server declares it, so `character varying(10)` against `character varying(50)` and `numeric(10,2)` against `numeric(12,4)` are differences, and so are two different enum types. On PostgreSQL a type in the table's own schema matches the other side's type of the same name: two schemas that each carry their own `status` enum compare equal. A type an extension owns is compared qualified on both sides, so a PostGIS column in `public` matches one in `staging`. A type the source's schema has and the target's does not is reported as a difference, and its `ALTER … TYPE` fails on the target with `type "…" does not exist`.
