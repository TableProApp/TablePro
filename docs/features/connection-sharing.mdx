...
Bring your connections over from TablePlus, Sequel Ace, DBeaver, or DataGrip. Passwords can be imported too. The source app doesn't need to be running.
...
DataGrip stores connections per project. TablePro imports the connections from the projects DataGrip is tracking (its recent projects), along with their SSH tunnel and SSL settings. If a connection is missing, open its project in DataGrip once so it returns to the recent list. DataGrip doesn't store SSH tunnel passwords where TablePro can read them, so re-enter those after importing. Connections protected by a DataGrip master password can't be read.
