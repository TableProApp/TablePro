...
On PostgreSQL, SQL Server and the other engines that keep objects in schemas, copy one schema at a
time. A whole-database copy taken without a schema reads only the connection's current one, so the
sheet asks for a schema instead of carrying part of the database and calling it the whole.

Views, routines and triggers copy only into a schema of the same name. Their definition is the
source's own SQL text and nothing rewrites the tables it names, so into a differently named schema
it would point back at the source. They are left out with the reason shown.
...
built parents first. Rows are copied after that, and triggers go in last: a trigger installed before
the rows fires on the copy itself.
