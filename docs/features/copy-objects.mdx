...
|---|---|---|
| MySQL, MariaDB | A key over 3,072 bytes, counting 4 bytes a character | Cuts the widest key column until the key fits: a primary key column to `VARCHAR(255)`, an index column to a 255-character prefix. An index that still does not fit is left out |
| MySQL, MariaDB | A row over 65,535 bytes, or over 8,126 bytes in one InnoDB record | Creates the widest column as `TEXT` until the table fits. Key columns never move, and indexed columns move last |
...
A primary key column cut to `VARCHAR(255)` refuses a row whose key is longer, and a unique index cut to
a prefix refuses two rows that agree for their first 255 characters. If your data holds such rows,
create the target table yourself with keys that fit, then use **Add rows to it**.
