...
A PostgreSQL column that uses an extension or user-defined type or function, such as a PostGIS
`geometry`, an enum, or a `DEFAULT` that calls `ST_GeomFromText`, is created naming each one in the
schema it lives in on the source: `public.geometry(Point,4326)`. A target database without them in
that schema refuses the table with `type "…" does not exist` or `function … does not exist`. Install
the extension or create the type there first, in the same schema.
