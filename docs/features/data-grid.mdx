...
When a row is selected, the inspector shows all column values with full (untruncated) content. Search columns by name. TEXT/VARCHAR columns get a multi-line editor. JSON/JSONB columns display auto-formatted JSON with syntax highlighting (colored keys, strings, numbers, and booleans). Edit values here; changes are queued like inline edits.
