...
Folders come from the same background menu. A folder's own menu has **New Favorite…**, **New Subfolder**, **Rename**, **Global**, and **Delete Folder**; deleting one moves what was inside up to the parent level. A global folder carries a globe, and marking a subfolder global marks the folders above it too. A global query kept in a folder that belongs to one connection is listed at the top level on the others. Drag a query or a linked file out of the sidebar to drop its SQL into the editor or another app.
