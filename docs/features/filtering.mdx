...
Autocomplete offers the table's columns and SQL keywords as you type a token, after AND and OR included. Closing a single-quoted string or parenthesis leaves the list closed so `Return` applies the filter; start typing the next token to see suggestions again. Arrow keys pick a suggestion, `Tab` or `Return` inserts it, `Escape` dismisses the list without closing the filter bar. On a database that does not speak SQL the row is labeled **Raw Filter** and takes that database's own syntax: on MongoDB, a filter document such as `{"customer.country": "US"}`.
