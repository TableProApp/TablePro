...
On SQL Server a file runs a batch at a time, cut at its `GO` lines the way `sqlcmd` cuts it, so every count and failure here is a batch rather than a statement. See [Scripts and batches](/databases/mssql#scripts-and-batches).
