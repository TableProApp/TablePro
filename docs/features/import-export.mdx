...
It opens the first format this connection's driver offers. **File > Import > Import Data From** beside it lists every format the driver has, so a second one is one click rather than a change of file extension. **Actions** in the toolbar carries the same pair.
