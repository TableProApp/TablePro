...
Caps: a document over 100,000 characters prints as a string rather than as nested keys, the same cap the [JSON tree](#limits) has. A binary value prints its first 64 bytes followed by an ellipsis; **Copy Value** carries all of it.
