...
**Open in Window** detaches a JSON, PHP or text value into its own resizable window; on an editable row, edits there join the same pending changes as the field. An image detaches read-only. The expand button on a JSON, PHP, text or binary field grows it to fill the inspector's height with the other fields still listed around it; click it again to shrink it back.
