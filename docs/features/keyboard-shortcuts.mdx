...
When Vim mode is enabled (**Settings** > **Editor** > **Editing** > **Vim mode**), the SQL editor uses modal keybindings. A mode indicator badge appears in the toolbar showing `NORMAL`, `INSERT`, `REPLACE`, `VISUAL`, or `VISUAL LINE`.

The full vim command surface is documented on the [Vim Mode](/features/vim-mode) page: motions (h/j/k/l, w/W/b/B/e/E/ge/gE, 0/$/^/_, gg/G, H/M/L, %, find-char f/F/t/T/;/,, sentence/paragraph ( )/{ }, [[ ]]), operators (d/c/y with motions, D/C/Y/S/s shorthands, ~/gu/gU/g~ case, >>/<< indent, J/gJ join), text objects (iw/aw, i"/a", i(/a(, i{/a{, it/at, ip/ap), search (/?/n/N/*/#), marks (m/'/`), named and numbered registers, macros (q/@/@@), repeat (.), insert-mode editing (Ctrl+W/U/H/T/D), number adjust (Ctrl+A/X), and scrolling (Ctrl+d/u/f/b, zt/zz/zb, gj/gk).
