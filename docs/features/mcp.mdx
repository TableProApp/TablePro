...
| Ask Each Time (default) | Approval dialog on first access per session |
| Never | Blocked |
...
| Destructive | DROP, TRUNCATE | Blocked. Use `confirm_destructive_operation` tool |

### Safe Mode

MCP queries follow the same Safe Mode as manual queries:

- **Silent** - no confirmation
- **Read-Only** - writes blocked
- **Alert** - confirmation dialog for writes
- **Safe Mode** - confirmation + Touch ID
