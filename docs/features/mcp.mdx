...
When the server starts, it picks a free port in the `51000-52000` range and writes a handshake file at `~/Library/Application Support/com.TablePro/mcp-handshake.json`. The `tablepro-mcp` CLI and the Raycast extension read this file to find the port; you do not need to configure it manually.

To start it manually or keep it running between launches, open **Settings > MCP** and toggle **Enable MCP Server**.
