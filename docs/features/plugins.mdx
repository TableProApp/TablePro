...
| Import formats | SQL, JSON, CSV, XLSX |
...
| Cloudflare R2 SQL | Cloudflare R2 SQL |
| DynamoDB | DynamoDB |
| BigQuery | BigQuery |
| Spanner | Spanner |
| Snowflake | Snowflake |
...
Saved connections survive either way. A connection whose driver cannot load stays in the list, and opening it lands on "Could not connect" with the button for the cause:

| Cause | Button |
|-------|--------|
| The driver is not installed | **Install Plugin…** downloads it and connects |
| The plugin is switched off | **Enable Plugin** switches it on and connects |
| The plugin failed to load or needs an update | **Open Plugin Settings** selects it under **Installed**. Click **Try Again** once it is fixed |
| The connection names a database type no plugin provides, which a connection imported from another app can | **Edit Connection…** opens the form to choose the type |

A plugin switched off stays off: connecting never switches it back on.
