...
## EXPLAIN plan captures

A successful EXPLAIN entry can also carry its raw plan output. Open that result's **History** sheet to compare it with earlier compatible runs; see [EXPLAIN Visualization](/features/explain-visualization#compare-earlier-plans).

The plan capture is attached to the ordinary history entry rather than stored as another row. It follows the same pause, retention and deletion rules: deleting an entry, clearing matching history, pruning it, or deleting its connection removes the plan capture too.

Raw plan output is limited to 2 MB per capture, 1,000 stored captures and 100 MB total. Crossing a total limit removes the oldest plan payloads while keeping their normal history entries. A larger individual result also keeps its normal history entry but skips the plan capture. Parameterized EXPLAIN results never persist their raw plan output.
