...
The **Queries** scope searches saved queries and the 200 most recent query-history entries from every connected window. Each row shows its connection and database. Recent queries also show their execution time. Global saved queries open in the connection where you opened Quick Switcher.

<Frame caption="Query history results from two open connections">
  <img src="/images/quick-switcher-cross-connection-queries.png" alt="Quick Switcher showing query history from Chinook and Analytics connections" />
</Frame>

Opening a query brings its connection forward and loads the SQL without running it. Query-history entries keep their recorded database context. If no open window uses that database, TablePro opens a new native window tab instead of loading the SQL into a tab for another database. `Option+Return` always opens a new native window tab.
