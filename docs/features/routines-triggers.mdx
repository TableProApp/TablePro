...
| MongoDB, Redis, Elasticsearch, Typesense, DynamoDB, etcd, SurrealDB, Trino, Beancount, Spanner | No | No | No |

A section appears only where the engine has that kind of object. An engine that has them and currently holds none keeps the section and shows **No items** inside it, so a database with no tables still lists its procedures and functions. A section whose list is still on its way shows **Loading…**, and one the server refused shows the error in place of its rows.
