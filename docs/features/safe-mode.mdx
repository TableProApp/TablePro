...
Six levels, one per connection, set in the **Options** section of its edit form. The level decides what stands between a query and the server: nothing, a confirmation, Touch ID, or a refusal.
...
1. **External Clients** (per connection: **Blocked** / **Read Only** / **Read & Write**), in the connection form's **Options** section. A Read Only connection rejects a write before the request reaches the database.
