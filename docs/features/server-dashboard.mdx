...
Open it from **Database > Server Dashboard**, or from **Server Dashboard** under **Actions** in the toolbar. A **Dashboard** button is in the palette too, through [Customize Toolbar](/features/connection-window#add-a-control-back).
...
On PostgreSQL 9.1 an idle session shows an empty Query: that server keeps only the running statement, not the last one.
...
|----------|:--------:|---------|:------------:|:------:|:---------:|
| PostgreSQL / Redshift | Yes | Connections, cache hit ratio, database size, uptime, active queries | Yes | Yes | Yes |
| CockroachDB | Yes | Connections, active queries | Yes | Yes | Yes |
...
Every other engine has no dashboard provider. The menu item dims, **Actions** leaves the entry out, and a **Dashboard** button you added to the toolbar dims.
