...
Open this when the question is about the server rather than about a table. It shows who is connected, whatever metrics the engine reports, and any query still running after a second, refreshing every 5 seconds until you change the interval or turn it off.

Open it from the menu bar **Database > Server Dashboard**. A Dashboard toolbar button is also available: right-click the toolbar, choose **Customize Toolbar**, and drag it in.
