...
A collapsed region is replaced by a chip showing the start of what it hides and the number of hidden lines, so a folded `CREATE TABLE` still tells you what is inside it. Click the chip to bring the region back. Pointing at a chip marks its chevron in the gutter too: the chip and the chevron are two ways to reach the same fold.
...
**Settings > Editor** covers line numbers, current-line highlight, word wrap, code folding, tab width, auto-uppercase keywords as you type, query parameters, and Vim mode. Editor font family and size are set per theme in **Settings > Appearance**.
