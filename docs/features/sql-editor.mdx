...
A SQL Server script runs a batch at a time instead, cut at its `GO` lines, so a `DECLARE`d variable reaches every statement after it. See [Scripts and batches](/databases/mssql#scripts-and-batches).
...
| One that manages its own: `BEGIN`, `START TRANSACTION`, `XA START`, `SET autocommit`, `SAVEPOINT` on SQLite, `SET TRANSACTION` on Oracle | The script's own transaction | The transaction it left open is rolled back |
...
| A script on SQL Server | No transaction, unless the script opens one with `BEGIN TRAN` | What ran stays applied, and a transaction the script left open is reported |
...
**Review** in the query bar sends the statement at the cursor, with the structure of the tables it names, for a review. Its menu holds **Explain with AI** (`Cmd+L`) and **Optimize with AI** (`Cmd+Option+L`), which send the same statement. When a query fails, **Fix with AI** in the error banner rewrites it. Inline suggestions complete your SQL as ghost text: `Tab` accepts, `Escape` dismisses. See [AI Assistant](/features/ai-assistant#review-explain-optimize-and-fix) for what each action sends.
