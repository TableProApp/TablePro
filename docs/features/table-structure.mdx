...
- **ClickHouse**: the Indexes tab drops a data-skipping index but cannot add or change one, because a row has no field for its expression, type or granularity. A save or a new table that tries is refused before anything runs. Run `ALTER TABLE … ADD INDEX … TYPE bloom_filter` in the SQL editor instead.
