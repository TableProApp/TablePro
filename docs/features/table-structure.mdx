...
Columns are edited in place. **Nullable**, **Primary Key**, and **Auto Inc** are YES/NO dropdowns; **Primary Key** set to YES forces **Nullable** to NO and holds it there until the key comes back off. **Type** opens a picker of the engine's types by category: search to filter, or type a parametric value such as `VARCHAR(255)` and press Return to use it as written. The cell reads the type as the server declares it, with its length, precision and type name: `character varying(50)`, `numeric(10,2)`, an enum or a domain under its own name. A type from another schema is qualified, and on PostgreSQL so is a type an extension owns: `public.geometry(Point,4326)`. Editing the cell writes back what was typed. On PostgreSQL and PGlite the picker opens with a **User-Defined** group of the database's enums, composites, domains and ranges, each listed schema-qualified as `sales.status`. See [User-Defined Types](/features/user-defined-types).
