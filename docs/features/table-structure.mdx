...
Drag a column row to reorder it. Dragging is off while unsaved changes exist, and off entirely on an engine that cannot change column order, where the row number carries a tooltip saying why.
...
| SQLite | Shows the rebuild script, and runs it when you confirm. The table is recreated in the wanted order, the rows are copied, and the indexes and triggers are put back |
| PostgreSQL, libSQL, Turso, Cloudflare D1 | Shows the rebuild script for you to read and run. TablePro does not run it: the script ends in `DROP TABLE`, and what it cannot carry over is listed above it |
