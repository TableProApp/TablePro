...
On the **Foreign Keys** grid, **Columns**, **Ref Table**, and **Ref Columns** open menus of the new table's own columns, the database's tables, and the referenced table's columns. Each menu ends in **Custom…** for a name the list does not carry, and picking a second entry in a comma-separated cell appends to it. **On Delete** and **On Update** list only the actions the engine takes: DuckDB accepts NO ACTION and RESTRICT, Oracle has no ON UPDATE, and SQL Server has no RESTRICT.
