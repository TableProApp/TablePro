...
The tabs are **Properties**, **Columns**, **Indexes**, **Foreign Keys**, **Constraints**, **Triggers**, **DDL**, and **Parts** (ClickHouse only); Columns through Triggers carry item counts. A tab the engine has no concept of is hidden: ClickHouse has no Foreign Keys, Redshift no Triggers, Redis no Constraints. Every grid has a filter field, and clicking a header sorts.
...
<Frame caption="Properties tab on a PostgreSQL table">
  <img className="block dark:hidden" src="/images/structure-properties.png" alt="A grouped form listing name, schema, owner and tablespace above a comment field holding several lines of JSON" />
  <img className="hidden dark:block" src="/images/structure-properties-dark.png" alt="A grouped form listing name, schema, owner and tablespace above a comment field holding several lines of JSON" />
</Frame>
...
- **Renaming a table** is not on the Properties tab. It happens in the sidebar row: see [Table Operations](/features/table-operations).
