...
- **ClickHouse**: the Indexes tab drops a data-skipping index but cannot add or change one, because the Type menu holds no ClickHouse index type. A save or a new table that tries is refused before anything runs. Run `ALTER TABLE … ADD INDEX … TYPE bloom_filter` in the SQL editor instead.
