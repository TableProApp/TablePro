...
Clicking a table opens it right away, in a preview tab that is reused when you click a different table, like VS Code's preview tabs. A tab with unsaved edits, an applied filter, or sorting is never replaced: the click opens a new tab instead. A preview tab becomes permanent when you interact with it (sort, filter, edit data), or straight away if you use **Open in New Tab** on the table's contextual menu. A preview tab still open at quit is restored as a permanent tab.
...
| New table tab | Click a table in the sidebar (the preview tab is reused), or **Open in New Tab** on its contextual menu for a permanent one |
